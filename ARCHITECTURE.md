...
*   **`config.py`**: Handles configuration loading using `Settings` dataclass, reading from environment variables (e.g., `POKERGPU_LOG_LEVEL`, `POKERGPU_DEVICE`).
...
### `src/pokergpu/eval/` (Hand Evaluation)
Handles calculating hand strength.
*   **`treys_evaluator.py`**: Interfaces with the `treys` library to evaluate poker hands (5-card or 7-card).
    *   *Key Classes:* `TreysHandEvaluator`.
...
4.  **Leaf Evaluation:** When a leaf node (terminal state or depth limit) is reached, the system must determine the expected value. This uses:
    *   The abstraction logic (`abstraction/` modules).
    *   Hand evaluation tools (`eval/treys_evaluator.py`).
