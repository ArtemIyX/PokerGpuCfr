...
class HeuristicLeafBackend(LeafEvalBackend):
    def evaluate(self, batch: LeafEvalBatchInput) -> LeafEvalBatchOutput:
        ...
        street_factor = np.asarray(street, dtype=np.float32)
        board_size_factor = np.asarray(board_size, dtype=np.float32)
        board_signature_factor = np.asarray(board_signature % 997, dtype=np.float32) / np.float32(997.0)

        raw_values = (
            (0.08 * reach)
            + (0.18 * share)
            + (0.05 * board_size_factor)
            + (0.06 * street_factor)
            + (0.12 * board_signature_factor)
            + (0.02 * leaf_index)
